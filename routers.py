...
async def websocket_alert_endpoint(
    websocket: WebSocket,
    group_id: int = None,  # if not None then widget is connected, else dashboard is connected
    ...
    alert_sequence_service.set_alert_sequence_data(key, alert_sequence_data)
    ...
    async def _stop_key_tasks():
        await alert_task_manager.stop_single_async_task((key, "state_machine"))
        await alert_task_manager.stop_single_async_task((key, "listen_rabbit"))
    ...
    moderation_settings_updated_at = ZERO_DATETIME
    ...
    alert_sequence_service.set_alerts(widget_token_info.author_id, pending_donations)
    message = WidgetMessage.make_pending_alerts_message(pending_donations)
    await websocket.send_json(message.model_dump(mode="json", by_alias=True))
    #########################################################

    ### CONNECTED GROUPS INFO
    ...
    await alert_task_manager.start_single_schedule_task(
        ...
        ws_alerts_manager.update_connected_groups_info,
        key,
        widget_token_info.author_id,
    ...
    await ws_alerts_manager.listen(
        key,
        websocket,
        get_ws_messages_handler(widget_token_info.author_id, exchange, alert_task_manager, ws_alerts_manager, key),
    )
